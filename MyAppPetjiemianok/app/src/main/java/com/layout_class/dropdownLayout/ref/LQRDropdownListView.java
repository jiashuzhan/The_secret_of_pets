package com.layout_class.dropdownLayout.ref;
...
public class LQRDropdownListView extends ScrollView {
    ...
    public LQRDropdownListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LQRDropdownListView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void flush() {
        for (int i = 0, n = linearLayout.getChildCount(); i < n; i++) {
            ...
            if (view instanceof LQRDropdownListItemView) {
                LQRDropdownListItemView itemView = (LQRDropdownListItemView) view;
                setDropdownListItemViewStyle(itemView);
                LQRDropdownItemObject data = (LQRDropdownItemObject) itemView.getTag();
                if (data == null) {
                    return;
                }
    ...
    public void bind(List<? extends LQRDropdownItemObject> list,
                     LQRDropdownButton button,
        ...
        LinkedList<LQRDropdownListItemView> cachedViews = new LinkedList<>();
        for (int i = 0, n = linearLayout.getChildCount(); i < n; i++) {
            View view = linearLayout.getChildAt(i);
            if (view instanceof LQRDropdownListItemView) {
                cachedViews.add((LQRDropdownListItemView) view);
            } else {
...
//        LayoutInflater inflater = LayoutInflater.from(getContext());
        ...
        for (LQRDropdownItemObject item : list) {
            if (isFirst) {
                isFirst = false;
            } else {
            ...
            LQRDropdownListItemView view = cachedViews.poll();
            if (view == null) {
            ...
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    LQRDropdownItemObject data = (LQRDropdownItemObject) v.getTag();
                    if (data == null) return;
                    LQRDropdownItemObject oldOne = current;
                    current = data;
                    flush();
                    container.hide();
                    if (oldOne != current) {
    ...
    private LQRDropdownListItemView genDropdownListItemView() {
        LQRDropdownListItemView view;
        view = new LQRDropdownListItemView(mContext);
    ...
    public static interface Container {
        void show(LQRDropdownListView listView);

        void hide();

        void onSelectionChanged(LQRDropdownListView view);
    }
    ...
    private void setDropdownListItemViewStyle(LQRDropdownListItemView itemView) {
        if (itemTextSize != -1)
