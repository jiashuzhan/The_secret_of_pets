package com.layout_class.dropdownLayout;
...
import com.layout_class.dropdownLayout.ref.LQRDropdownButton;
import com.layout_class.dropdownLayout.ref.LQRDropdownItemObject;
import com.layout_class.dropdownLayout.ref.LQRDropdownListView;
...
public class LQRDropdownLayout extends FrameLayout {
    ...
    //下拉按钮集合
    private List<LQRDropdownButton> mDropdownButtonList = new ArrayList<>();
    //下拉列表集合
    private List<LQRDropdownListView> mDropdownListViewList = new ArrayList<>();
    //下拉列表中所有列的的key和value的集合
    private List<Map<String, String>> mDropdownListStringData;
    //下拉列表所有列的DropdownItemObject集合的集合
    private List<List<LQRDropdownItemObject>> mAllDropdownListData = new ArrayList<>();
    ...
    public LQRDropdownLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LQRDropdownLayout);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.LQRDropdownLayout_ddl_only_show_one) {
                onlyShowOne = typedArray.getBoolean(attr, false);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_cols) {
                cols = typedArray.getInt(attr, 1);

                //下拉按钮
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_text_prefix) {
                topBtnTextPrefix = typedArray.getString(attr);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_text_suffix) {
                topBtnTextSuffix = typedArray.getString(attr);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_bg) {
                topBg = typedArray.getColor(attr, Color.WHITE);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_height) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_split_line_color) {
                topSplitLineColor = typedArray.getColor(attr, Color.LTGRAY);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_split_line_width) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_split_line_height) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_text_size) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_text_normal_color) {
                topTextNormalColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_text_selected_color) {
                topTextSelectedColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_noraml_drawable) {
                topNormalDrawableResId = typedArray.getResourceId(attr, -1);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_btn_selected_drawable) {
                topSelectedDrawableResId = typedArray.getResourceId(attr, -1);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_bottom_line_color) {
                topBottomLineColor = typedArray.getColor(attr, Color.LTGRAY);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_bottom_line_width) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_top_bottom_line_height) {
                ...
                //分割线
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_split_line_color) {
                splitLineColor = typedArray.getColor(attr, Color.LTGRAY);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_split_line_width) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_split_line_height) {
                ...
                //遮盖层
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_mask_bg) {
                maskBg = typedArray.getColor(attr, Color.GRAY);

                //下拉列表最大高度
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_list_max_height) {
                ...
                //item
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_padding_left) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_padding_right) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_height) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_text_size) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_text_normal_color) {
                itemTextNormalColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_text_selected_color) {
                itemTextSelectedColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_normal_bg) {
                itemNormalBg = typedArray.getColor(attr, Color.WHITE);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_selected_bg) {
                itemSelectedBg = typedArray.getColor(attr, Color.GRAY);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_normal_drawable) {
                itemNormalDrawable = typedArray.getResourceId(attr, -1);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_selected__drawable) {
                itemSelectedDrawable = typedArray.getResourceId(attr, -1);

            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_text_gravity) {
                itemTextGravity = typedArray.getInt(attr, -1);

                //item下划线
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_bottom_line_height) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_bottom_line_margin_left) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_bottom_line_margin_right) {
            ...
            } else if (attr == R.styleable.LQRDropdownLayout_ddl_item_bottom_line_color) {
    ...
    public class DropdownButtonsController implements LQRDropdownListView.Container {
        ...
        @Override
        public void show(LQRDropdownListView view) {
        ...
        @Override
        public void onSelectionChanged(LQRDropdownListView view) {
            if (mOnDropdownListListener != null) {
                int indexOf = mDropdownListViewList.indexOf(view);
                LQRDropdownItemObject current = view.current;
         ...
         */
        void reset() {
            for (LQRDropdownButton button : mDropdownButtonList) {
                button.setChecked(false);
            }
            for (LQRDropdownListView listView : mDropdownListViewList) {
        ...
        public void transToItemObjectList() {
            ...
            for (Map<String, String> map : mDropdownListStringData) {
                List<LQRDropdownItemObject> dataSet = new ArrayList<>();
                int i = 0;
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    dataSet.add(new LQRDropdownItemObject(entry.getKey(), i++, entry.getValue()));
                }
